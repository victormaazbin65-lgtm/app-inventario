(function (global) {
    ...
    async function exportarOrden(){const o=ordenActual();if(!o.items.length)return alert('No hay productos en la orden.');const filas=o.items.map(i=>({Codigo:i.codigo,Producto:i.nombre,Proveedor:i.proveedor,Stock:i.stock,Minimo:i.minimo,Comprar:i.cantidad,CostoUnitario:i.costoUnitario,Subtotal:i.subtotal}));try{if(!global.XLSX&&typeof global.asegurarXLSX==='function')await global.asegurarXLSX();}catch(_){}if(global.XLSX){const ws=global.XLSX.utils.json_to_sheet(filas);const wb=global.XLSX.utils.book_new();global.XLSX.utils.book_append_sheet(wb,ws,'Orden');global.XLSX.writeFile(wb,`Orden_${core.claveUnica(o.proveedor)}_${new Date().toISOString().slice(0,10)}.xlsx`);return;}const cab=Object.keys(filas[0]);const csv=[cab.join(','),...filas.map(f=>cab.map(k=>`"${String(f[k]??'').replaceAll('"','""')}"`).join(','))].join('\n');descargar(`Orden_${core.claveUnica(o.proveedor)}.csv`,csv,'text/csv;charset=utf-8');}
    ...
    function pestañaActivaOperaciones(nombre){return Boolean(document.getElementById('tab-'+nombre)?.classList.contains('active'));}
    function campoEdicionActivo(){const activo=document.activeElement;return Boolean(activo&&typeof activo.matches==='function'&&activo.matches('input, textarea, select, [contenteditable="true"]'));}

    function refrescar(){
        if(document.visibilityState==='hidden'||campoEdicionActivo())return;
        if(pestañaActivaOperaciones('inicio')){asegurarCierreDiario();renderCierre();}
        if(pestañaActivaOperaciones('alertas')){asegurarOrdenCompra();actualizarOrdenCompra();}
        if(pestañaActivaOperaciones('inventario')){asegurarEtiquetas();actualizarEtiquetas();}
        if(pestañaActivaOperaciones('ajustes'))asegurarPanelAuditoria();
    }

    function instalarRefrescoPorPestana(){
        const original=global.cambiarPestaña;
        if(typeof original!=='function'||original.__v130opsRefresh)return;
        const envuelta=function(...args){const r=original.apply(this,args);setTimeout(refrescar,80);return r;};
